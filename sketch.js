...
function startColaHistoryNavigationFade(
    kind,
    index
) {
    if (
...
function completeColaHistoryNavigationFade(
    fade
) {
    if (
        !gameState ||
...
function drawColaHistoryNavigationFade() {
    const fade =
        gameState &&
        gameState.historyNavigationFade;
    ...
    if (
        ...
        opacity =
            colaHistoryClampFade(
                elapsed /
                    fade.outDuration
            );

        if (opacity >= 1) {
            completeColaHistoryNavigationFade(
                fade
            );
...
const drawBaseForColaHistoryNavigationFade =
    draw;

draw = function() {
    const result =
        drawBaseForColaHistoryNavigationFade.apply(
            this,
            arguments
        );

    drawColaHistoryNavigationFade();

    return result;
};

const touchedBaseForColaHistoryNavigationFade =
    touched;
...
touched = function(touch) {
    if (
        ...
        return touchedBaseForColaHistoryNavigationFade.apply(
            this,
            arguments
        );
    }
    ...
    if (
        ...
        startColaHistoryNavigationFade(
            "title_to_history"
        );
...
function colaHistoryFinalFadeNow() {
    if (
        typeof ElapsedTime !==
        "undefined"
    ) {
...
function colaHistoryFinalFadeClamp(
    value
) {
    return Math.max(
        0,
...
function startColaHistoryFinalFade(
    kind,
    index
) {
    if (
    ...
    gameState.colaHistoryFinalFade = {
        ...
        stage: "out",
        startedAt:
            colaHistoryFinalFadeNow(),
        outDuration: 0.20,
        inDuration: 0.24,
    };
}

function commitColaHistoryFinalFade(
    fade
) {
    if (
        !gameState ||
...
function drawColaHistoryFinalFade() {
    const fade =
        gameState &&
        gameState.colaHistoryFinalFade;
    ...
    if (
        ...
        opacity =
            colaHistoryFinalFadeClamp(
                elapsed /
                    fade.outDuration
            );

        if (opacity >= 1) {
            commitColaHistoryFinalFade(
                fade
            );
...
const drawBaseForColaHistoryFinalFade =
    draw;

draw = function() {
    const result =
        drawBaseForColaHistoryFinalFade.apply(
            this,
            arguments
        );

    drawColaHistoryFinalFade();

    return result;
};

const touchedBaseForColaHistoryFinalFade =
    touched;
...
touched = function(touch) {
    if (
        ...
        return touchedBaseForColaHistoryFinalFade.apply(
            this,
            arguments
        );
    }
